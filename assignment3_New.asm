...
#	10 Nov, 2016
#	
#	PURPOSE:   This program will execute a tic tac toe game with several methods.
#			
#	INPUTS:	The inputs of this program are user inputs for moves
#
#	OUTPUTS: The state of the board, wins, losses, and draws.
#
#############################################################################################
#################################   Data Segment    ######################################### 
.data
...
start:								#
      								#
      lb $t0, 0($k0)				# public void makeBoard(){
      lb $t1, 1($k0)				#	for (row = 0; row < 3; row++)
      lb $t2, 2($k0)				#		for (column = 0; column < 3; column++)
      lb $t3, 3($k0)				#			board[row][column] = '_';}
      lb $t4, 4($k0)				#
      lb $t5, 5($k0)				#
      lb $t6, 6($k0)				#
      lb $t7, 7($k0)				#
...
IsAWin:                         	#This function will return
        ...
        lb $t3, 3($k0)				#
        lb $t4, 4($k0)				#The registers are loaded into a temp register to 
        lb $t5, 5($k0)				#look/compare the values of each (whether it's a X or O). 
        lb $t6, 6($k0)				#Then, it will then
        lb $t7, 7($k0)				#
        lb $t8, 8($k0)				#
        bne $t0, $t1, C2			#
        bne $t1, $t2, C2			#
        ...
        jr $ra						#
C2:									# 
        bne $t3, $t4, C3			# private boolean rowWin(){
        bne $t4, $t5, C3			# 	for (int i = 0; i < 3; i++){
        addi $v0, $zero, 1			#		if (board[i][0] == board[i][1] && board[i][1] == board[i][2] && board[i][0] != '_')
        jr $ra						#			return true;	
C3:									#		}
        bne $t6, $t7, C4			#		return false;}
        bne $t7, $t8, C4			# private boolean columnWin(){
        addi $v0, $zero, 1			#	for (int j = 0; j < 3; j++){
        jr $ra						#		if (board[0][j] == board[1][j] && board[1][j] == board[2][j] && board[0][j] != '_')
C4:									#			return true;	
        bne $t0, $t3, C5			#	}
        bne $t3, $t6, C5			#	return false;}
        addi $v0, $zero, 1			# private boolean diagonalWin(){
        jr $ra						#	if ((board[0][0] == board[1][1] && board[1][1] == board[2][2] && board[0][0] != '_') || 
C5:									#			(board[0][2] == board[1][1] && board[1][1] == board[2][0]&& board[0][2] != '_')){
        bne $t1, $t4, C6			#		return true;}
        bne $t4, $t7, C6			#	return false;}
        addi $v0, $zero, 1			#
        jr $ra						#
C6:									# private boolean winCheck(){
        bne $t2, $t5, C7			#	return(rowWin() || columnWin() || diagonalWin() == true);
        bne $t5, $t8, C7			# }
        addi $v0, $zero, 1			#
        jr $ra						#
C7:									#
...
DisplayBoard:       				#This function will display the board
     ...
     syscall						#
     								#
B1:  								#B1-B9, will place the numbers into each square of the board, 
     addi $a0, $t0, 0				#so top left is 1, top middle is 2, etc.
     addi $v0, $zero, 11			#
     syscall						#
    								#
     la $a0, b3						#
